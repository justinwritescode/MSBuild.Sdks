...
{
    public class ComparersImplementation : IEqualityComparer<MSBEx.ProjectPropertyInstance>, IEqualityComparer<MSBEx.ProjectItemInstance>, IEqualityComparer<XElement>, IEqualityComparer<XAttribute>, IEqualityComparer<XElementOrProjectItemInstance>
    {
        public static TaskLog? Logger { get; set; }

        public bool Equals(ProjectPropertyInstance? x, ProjectPropertyInstance? y) => x.Name.Equals(y.Name, StringComparison.InvariantCultureIgnoreCase) && x.EvaluatedValue.Equals(y.EvaluatedValue, StringComparison.InvariantCultureIgnoreCase);
        ...
        public bool Equals(ProjectItemInstance? x, ProjectItemInstance? y)
            => GetComparisonString(x).Equals(GetComparisonString(y), StringComparison.InvariantCultureIgnoreCase);
        ...
        public bool Equals(XElement? x, XElement? y) => GetComparisonString(x).Equals(GetComparisonString(y), StringComparison.InvariantCultureIgnoreCase);

        public int GetHashCode(XElement? obj) => GetComparisonString(obj).GetHashCode();

        public bool Equals(XAttribute? x, XAttribute? y) => x.Name.ToString().Equals(y.Name.ToString(), StringComparison.InvariantCultureIgnoreCase) && (x.Value?.ToString()?.Equals(y.Value?.ToString(), StringComparison.InvariantCultureIgnoreCase) ?? false);

        public int GetHashCode(XAttribute? obj) => (obj.Name + obj.Value).GetHashCode();

        int IEqualityComparer<XElementOrProjectItemInstance>.GetHashCode(XElementOrProjectItemInstance obj) => obj.IsFirst ? GetHashCode(obj.First) : GetHashCode(obj.Second);
    }
}
